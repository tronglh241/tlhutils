...
class Camera(ABC):
    def __init__(
        ...
        pix_coords = xy.reshape(2, -1).T.to(device)

        assert extrinsic.shape[-2:] == (4, 4), extrinsic.shape[-2:]
        extrinsic = extrinsic.to(device).to(torch.float32)
    ...
    def cam_to_world(self, points_3d: torch.Tensor) -> torch.Tensor:
        # points_3d (..., 3)
        assert points_3d.shape[-1] == 3, points_3d.shape
        points_3d = points_3d.to(self.device)
        org_shape = points_3d.shape

        points_3d = torch.cat((points_3d, torch.ones((*org_shape[:-1], 1), device=self.device)), dim=-1)
    ...
    def im_to_cam_map(
        ...
        *pre_dims, height, width, n = depth_map.shape
        ...
        valid_points = torch.where(mask > 0.5, valid_points, False)
    ...
    def _in_fov(
        ...
        return torch.logical_and(
            fov[..., 0:1] <= torch.arctan2(x1, x2),
            torch.arctan2(x1, x2) <= fov[..., 1:2],
        )
